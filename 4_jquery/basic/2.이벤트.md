...
3. id="btn1" 클릭 했을때 id="textZone" 글자 색상을 파란색으로 변경
   ```js

   ```
4. id="btn2"에 마우스 오버 했을때 id="textZone" 배경 색상을 노란색으로 변경
5. id="btn2"에 포커스가 생겼을때 id="textZone" 배경 색상을 노란색으로 변경
6. 두개이상의 이벤트(mouseover,click,focus)를 등록시킬때 \*
7. id="listWrap"에 마우스가 올라가 있으면 class="list1" 을 블록요소 변경합니다. mouseover와 비슷
8. id="listWrap"에서 마우스가 벗어나면 class="list1" 을 블록요소 변경합니다. mouseout과 비슷
9. class="hover"인 요소에 마우스를 올렸을땐 함수1을 벗어났을때에는 함수2를 실행 합니다
