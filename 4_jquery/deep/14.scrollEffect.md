...
> 위의 코드는 화면에는 보이지 않지만 버그가 있다. `['.mockup.pc', '.mockup.mobile']` 에는 단일 요소가 할당 되었으므로두개의 mockup.pc 가 추가 될 경우 각각 동작하지 않는다. 이것을 제이쿼리 객체로 변경하면 여러개 요소를 알아서 반복 처리 하기 때문에 각각 다른 이벤트를 처리할수 있다.
>아래처럼 수정하자
>
