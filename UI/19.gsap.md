...
==- [!badge variant='info' size='s' text='07'] data speed를 활용한 패럴렉스
...
// data-speed 속성을 취득하여 각 요소에 반영한다.
gsap.to('[data-speed]', {
...
2. `(i, el) =>`
   ...
   - i(인덱스)와 el(요소)를 매개변수로 받는다. el(요소)만을 사용하여 data-speed 속성 값을 가져온다.

3. `( 1- parseFloat(el.getAttribute('data-speed'))`

   - el의 data-speed 속성 값은 문자열 형태로 저장되어 있기 때문에 숫자로 변환해야 연산에 사용할 수 있다. `parseFloat` 함수는 문자형을 숫자형으로 변환하며 소수점 자리 까지 반환받는다
   - 1에서 data-speed 속성 값을 뺀 결과를 반환한다. data-speed 값이 1에 가까울수록 결과는 0에 가까워지며, data-speed 값이 0에 가까울수록 결과는 1에 가까워진다.

4. `ScrollTrigger.maxScroll(window)`
   - 스크롤이 가능한 최대 범위를 반환하며 전체 윈도우의 스크롤 가능 범위를 의미한다.
...
[!button variant='primary' icon='play' text='실행화면' target='blank'](https://qwerewqwerew.github.io/source/js/deep/gsap/07-horizontal.html)
